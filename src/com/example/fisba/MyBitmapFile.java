...
public class MyBitmapFile {
	...
	public MyBitmapFile(int nWidth, int nHeight, byte[] pImage) {
		...
		int length = m_fileHeaderBitmap.sizeof() + m_infoBitmap.sizeof()
				+ nWidth * nHeight;
		m_fileHeaderBitmap.bfSize = length;
		m_fileHeaderBitmap.bfOffBits = m_fileHeaderBitmap.sizeof()
				+ m_infoBitmap.sizeof();
		...
		for (int iCyc = 0; iCyc < nHeight; iCyc++) {
			System.arraycopy(pImage, (nHeight - iCyc - 1) * nWidth,
					pRotateImage, nImgOffset, nWidth);
			nImgOffset += nWidth;
		}
